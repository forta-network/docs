...
- **Labels.** A persistent tag placed on an entity (smart contract or EOA) responsible for a scam or attack. Today, the Scam Detector assigns one label - “scam”. Labels also feature the threat type (ex: ice phishing), the justification for the label and a confidence score from 0 to 1. 
...
The Scam Detector currently monitor on-chain activity for the following threat types:
