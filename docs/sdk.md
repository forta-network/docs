...
A utility method for verifying the claims and signature of a JWT generated by a scan node. This method verifies that the JWT was generated and signed by the same scan node the bot is running on.

- `token` - **required**  a JWT token generated by `fetchJwtToken`
